...
ctx.drawImage(shop, 118 * shopFrame, 0, 118, 128, canvas.width * 3 / 4 - 118, canvas.height, 118, 128)
const gravity = 10
const fps = 50
...
class Sprite {
    constructor({position, velocity, color = "red", name}){
        ...
        for(var i in this.images){
            ...
            this.images[new String(this.images[i].state) + "_" + "left"].image.src = this.images[new String(this.images[i].state) + "_" + "left"].imageSrc
        }
    }
    ...
    draw(state = 'Idle', enemy){
        ...
        ctx.drawImage(
            ...
            this.position.y - (50 * (this.width * 10 / this.height)) + (this.name == 'samuraiMack' ? 10 : 0),
            this.width * 10,
            this.height * (this.width * 10 / this.height)
        )
        this.frameElapsed++
        if(this.frameElapsed % this.frameHold === 0){
            if (this.direction === "right" && this.frame.x < noFrame - 1) {
                this.frame.x++
            } else if(this.direction === "left" && this.frame.x > 0){
                this.frame.x--
            } else {
                if(this.direction === 'right'){
                    this.frame.x = 0
                ...
                if(this.isAttack){
                    ...
                    if(
                        (
                            ...
                            (
                                this.attackBox.position.x - this.attackBox.width + this.width < enemy.position.x + enemy.width &&
                                this.attackBox.position.x - this.attackBox.width + this.width + this.attackBox.width > enemy.position.x &&
                                this.attackBox.position.y < enemy.position.y + enemy.height &&
                                this.attackBox.position.y + this.attackBox.height > enemy.position.y &&
                                this.direction === "left"
                            )
                        ...
                        if((e.offsetWidth - (document.querySelector(`${this.name == 'samuraiMack' ? ".enemy-hp" : '.player-hp'}`).offsetWidth * 10 / 100)) > 0) {
                            e.style.width = (e.offsetWidth - (document.querySelector(".enemy-hp").offsetWidth * 10 / 100)) + "px"
                        } else {
                            e.style.width = 0 + "px"
                            gameOverF(`Player ${this.name == "samuraiMack" ? 1 : 2} win \n press Q to play again.`)
                        }
                    }
                }
            }
    ...
    update(){
        if(!keys[87] && !keys[68] && !keys[65] && !keys[32] && this.jump == 'endJump' && !this.isAttack){
            this.draw()
        }        
...
enemy.update = () => {
    if(!keys[38] && !keys[39] && !keys[37] && !keys[13] && enemy.jump == "endJump" && !enemy.isAttack){
        enemy.draw()
    }
...
function animate(timeStamp){
    ...
    if(deltaTime > interval){
        ...
        ctx.drawImage(shop, shopFrame * shop.width / 6, 0, shop.width / 6, shop.height, canvas.width * 3 / 5, canvas.height - shop.height * (shop.width / 6 * 4 / shop.height) - 5, (shop.width / 6) * 4, shop.height * (shop.width / 6 * 4 / shop.height))
